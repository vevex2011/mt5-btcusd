...
#property description "BTCUSD MT5 model-direct execution EA with ATR risk management and TSV trade logging."
...
input bool            InpUseHigherTimeframeTrendFilter = true;
input ENUM_TIMEFRAMES InpTrendFilterTimeframe = PERIOD_H4;
...
input int             InpMaxPositionsPerDirection = 2;
input double          InpAddOnMinProfitR     = 0.80;
input int             InpStopLossCooldownBars = 4;
input ulong           InpMagicNumber          = 2026050601;
input double          InpRiskPerTradePercent  = 0.30;
input int             InpFastMAPeriod         = 20;
input int             InpSlowMAPeriod         = 50;
input int             InpRSIPeriod            = 14;
input int             InpATRPeriod            = 14;
input double          InpStopLossATR          = 1.80;
...
input double          InpProfitLockTriggerR   = 0.90;
input double          InpProfitLockAmountR    = 0.20;
input bool            InpRemoveTakeProfitOnLock = false;
input double          InpRemoveTakeProfitTriggerR = 1.20;
...
input double          InpMaxFallbackRiskPercent = 1.00;
input bool            InpShowIndicators       = false;
input string          InpLogFolder            = "codex-mt5-btcusd";
...
input bool            InpAllowTradeWhenModelMissing = true;
...
string   last_signal_setup = "MODEL_DIRECT";
...
bool IsDailyLossLimitHit()
{
   ...
   double loss_limit = AccountInfoDouble(ACCOUNT_BALANCE) * (InpMaxDailyLossPercent / 100.0);
   return(today_closed_pnl <= (-1.0 * loss_limit));
}
