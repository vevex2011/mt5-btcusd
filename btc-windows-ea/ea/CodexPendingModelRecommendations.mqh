...
bool GetModelExecutionPlan(const TradeSignal signal,
                           ...
                           string &reason)
{
   risk_percent = InpRiskPerOrderPercent;
   stop_atr = InpStopLossATR;
   target_atr = InpStopLossATR * 1.5;
   max_positions_per_direction = InpMaxModelDirectPositionsPerDirection;
   cooldown_minutes = 0;
   reason = "";
...
int CountModelStrategyPositions(const TradeSignal signal)
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; --i)
   {
      ...
      if(magic != InpModelDirectMagicNumber)
         continue;
...
string ModelLastEntryKey(const TradeSignal signal)
{
   return("CodexModelLastEntry." +
          IntegerToString((long)InpModelDirectMagicNumber) +
          "." + ConfiguredSymbol() +
          "." + SignalToText(signal));
}
